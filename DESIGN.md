...
## 1. Prinsip Desain# DESIGN — Laundrie
...
6. **Flat & ringan.** Prioritaskan tonal layering dan border tipis daripada shadow berat. Transisi 150–300ms, hanya pada properti warna/opacity/transform (jangan layout-shifting).
7. **Trust & Authority.** Setiap layar "konversi" (landing, login, checkout, settlement) menampilkan bukti kredibilitas: badge verified, statistik, riwayat bukti, nomor order — bukan janji kosong.
8. **Aksesibel secara default.** Kontras teks ≥4.5:1, fokus keyboard selalu terlihat, error bukan hanya warna, mendukung `prefers-reduced-motion`.
...
**Aturan kontras:** teks normal ≥4.5:1 terhadap latar; teks besar / elemen non-teks (border, ikon bermakna) ≥3:1. Jangan pernah menempatkan `on-surface-variant` (#3F484D) pada `surface-variant` (#DBE4E8) untuk teks kecil (rasio <4.5:1) — gunakan `on-surface` bila perlu.
...
### 2.6 Dark Mode (target v1.1+)

Dark mode **didukung desain** (dua pasang token), bukan sekadar invert.
- `surface` → `#121617`, `surface-container` → `#1D2426`, `background` → `#0E1113`.
- `on-surface` → `#E1E6E8` (kontras ≥4.5:1 terhadap surface gelap).
- `primary` tetap teal terang `#4FB3C7` untuk CTA; gunakan variant terang `#A6D8E3` untuk container.
- **Jangan** memakai nilai light mode pada latar gelap — setiap token diukur ulang kontrasnya.
- Status semantik juga punya varian gelap (success `#81C784`, warning `#E3C566`, error `#FF8A80`).
- State hover/focus/disabled diuji di kedua mode, bukan asumsi dari satu mode.

### 2.7 Motion

Micro-interaction **subtle**, bukan choreography besar:
- Durasi default **150–300ms**, easing `ease-out` untuk masuk, `ease-in` lebih cepat untuk keluar (keluar lebih cepat dari masuk).
- Animasi hanya pada warna/opacity/transform — **tidak pernah** me-restart layout (jitter).
- Tombol ditekan: opacity/elevation turun dalam ≤150ms.
- Loading: skeleton stabil atau spinner kecil dengan `aria-busy="true"`; jangan spinner yang berkedip untuk operasi <300ms.
- Hormati `prefers-reduced-motion`: matikan gerakan non-esensial, CTA tetap statis.
...
### 3.1 Buttons
- **Primary:** `primary` solid, teks `on-primary`, radius 12px, weight 700, tinggi minimum 48px (area tap ramah jari), `cursor-pointer`.
- **Secondary:** outline `primary` atau `primary-container`, teks `on-primary-container`.
- **Disabled:** `surface-variant` dengan teks `on-surface-variant` pada 60% opacity, `cursor-not-allowed`, non-interaktif (`disabled`).
- **Loading state:** spinner kecil inline + teks "Memproses…", `aria-busy="true"`, tombol di-disable.
...
**State error:** border `error`, teks error di bawah input dengan `role="alert"`/`aria-live` dan `aria-describedby` menghubungkan input ke pesannya; `aria-invalid="true"` pada input.

### 3.10 Form / Autentikasi (Login, Registrasi)
- **Password visibility toggle** (lihat/sembunyikan) — jangan pernah kunci password. Wajib mengizinkan paste & password manager (`autocomplete="current-password"` / `"new-password"`).
- **Error summary** di atas form: container `role="alert"` + `tabindex="-1"`, fokus dipindah ke sana setelah submit gagal; tiap item adalah link ke field yang bermasalah (`#email`, `#password`). Inline error di tiap field tetap dipertahankan.
- **Inline validation on blur** untuk field penting (format email, panjang password) — jangan hanya validate saat submit.
- **Submit feedback:** tombol loading → success/error. Jangan diam tanpa respon.
- Heading halaman jelas ("Selamat datang kembali" / "Buat akun baru") + subteks ringkas; aksi sekunder (daftar/login, lupa password) selalu terlihat.
- Panel brand di sisi kiri (desktop) menampilkan value proposition + bukti kredibilitas (badge terverifikasi, poin kepercayaan) — memakai `primary` dan `primary-container`.
...
- **Form login/register:** kartu center max-width 448px pada mobile & desktop; panel brand 50% lebar hanya di ≥1024px.
...
- Komponen bersama (badge, button, card, step indicator, field, alert) di `apps/web/src/shared/`; komponen fitur di `apps/web/src/features/*/`.
- **Fokus keyboard:** `:focus-visible` ring `primary` pada semua kontrol; jangan hapus outline.
- **Semantik:** gunakan `role`, `aria-*`, `autocomplete` yang benar; ikon dekoratif diberi `aria-hidden="true"`.
- **Kursor:** semua elemen yang bisa diklik diberi `cursor-pointer`.
...
## 7. Checklist Aksesibilitas (WCAG 2.2 AA)

Berlaku untuk semua layar:

- [ ] Kontras teks normal ≥4.5:1; teks besar / non-teks ≥3:1.
- [ ] Setiap kontrol punya label dan fokus `:focus-visible` yang terlihat.
- [ ] Error bukan hanya warna: selalu ada teks + `role="alert"`/`aria-live`.
- [ ] Form gagal: inline error dipertahankan + error summary fokus & ber-link.
- [ ] `autocomplete` benar (name, email, tel, current-password, new-password).
- [ ] Autentikasi mengizinkan paste & password manager; ada alternatif non-kognitif (passkey/OAuth) untuk v1.1+.
- [ ] Ikon dekoratif `aria-hidden="true"`; ikon bermakna punya teks alternatif.
- [ ] Touch target ≥44×44pt (iOS) / ≥48×48dp (Android).
- [ ] `prefers-reduced-motion` dihormati; konten otomatis punya kontrol pause.
- [ ] Tabel/angka invoice memakai `font-variant-numeric: tabular-nums`.
